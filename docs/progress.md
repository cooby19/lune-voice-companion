...
| M2 | 已實作（本機 public gate；待 push／CI） | 17 項 M2 測試；immutable revision／逐檔 SHA-256；final-only typed event；四層 generation fence；bounded latest-wins pending；lazy optional import 與 bounded close |
...
M2 本機 public gate 已完成；GitHub CLI 需重新登入後才能 push／確認 CI，local model／私人
語料 gate 也尚未執行。完成遠端 gate 後，後續工作請先閱讀
