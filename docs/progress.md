...
| M2 | 已實作（本機 public gate；已 push、CI run 未建立） | 17 項 M2 測試；immutable revision／逐檔 SHA-256；final-only typed event；四層 generation fence；bounded latest-wins pending；lazy optional import 與 bounded close；[commit `ebe262d`](https://github.com/cooby19/lune-voice-companion/commit/ebe262d1fc588351d1b2598d23cfaa9eb48dca8e) |
...
M2 本機 public gate 已完成且 commit 已 push；GitHub Actions API 尚未為該 push 建立 run，
local model／私人語料 gate 也尚未執行。完成遠端 gate 後，後續工作請先閱讀
[`handoff-m2-m8.md`](handoff-m2-m8.md)，並從 M3 開始。
