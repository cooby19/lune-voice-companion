...
### 端到端延遲拆解：Whisper 才是主要瓶頸（2026-08-28 實測）

四個環節首次全部有實測值（目標機 MacBook Air M4／16 GB）：

| 環節 | p50 | p95 | 來源 |
|---|---|---|---|
| 句尾靜音 | 350 ms | 350 ms | 固定值 |
| Whisper final | 1,959 ms | 2,076 ms | 實測，pinned `large-v3-turbo-q4` |
| LLM 首句（第一句限長） | 1,160 ms | 1,804 ms | 實測，`Qwen3.5-4B` Q4 |
| TTS TTFA（AVSpeech） | 28 ms | 328 ms | 實測 |
| **合計** | **約 3,500 ms** | **約 4,560 ms** | |

Whisper 延遲**幾乎與音訊長度無關**：1.46 s 音訊需 1,977 ms，6.23 s 音訊需 2,098 ms，
全部落在 1,909–2,178 ms。這是 Whisper 固定 30 秒 mel window 的架構特性，不是量測誤差。

由此得到一個與 LLM 選擇無關的結論：**原訂端到端 p50 ≤1.5 s 在此硬體上不可能達成**。
光是 350 ms 靜音加上約 1,950 ms 的 Whisper 就已是 2,300 ms，此時 LLM 與 TTS 尚未開始。
即使把 LLM 換成延遲為零的雲端服務，端到端仍至少約 2,330 ms。
